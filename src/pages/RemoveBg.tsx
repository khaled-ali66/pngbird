...
import { Loader2, Download, Image as ImageIcon, Sparkles, ArrowRight, UploadCloud, Key, Info, Crown, Wand2, X, Zap } from 'lucide-react';
...
export default function RemoveBg() {
  ...
  const handleGenerate = async () => {
    ...
    // Check for mandatory key selection if using premium models
    ...
    try {
      ...
      if (technique === 'Auto') {
        ...
        const blob = await rb(sourceImage, {
          model: isMobile ? "isnet_quint8" : "isnet_fp16", // quint8 is much safer and faster for mobile CPUs
          device: isMobile ? "cpu" : "gpu", // Force CPU on mobile to prevent WebGL corruption
          proxyToWorker: true, // Ensure it runs in a Web Worker to prevent UI freezing
          output: {
            format: "image/png",
            quality: 1.0
          },
        ...
        if (!apiKey && !isPremiumModel) {
          ...
          if (sourceImage.startsWith('http') || sourceImage.startsWith('//')) {
            // Ensure we use https for protocol-relative URLs
...
Ensure physically correct lighting and sharp edges.` }
          ...
          if (!whiteRes.candidates?.[0]?.content?.parts) {
            throw new Error("The AI model didn't return any results.");
          }
...
The edges should show thin bright highlights and subtle refraction, not solid color.` }
      ...
      if (resultUrl) {
        ...
        if (user) {
          try {
            await storage.saveImage(user?.id || '', { url: resultUrl, prompt: 'Background Removed' });
          } catch (err) {
            console.error('Error saving generation:', err);
          }
        }

        // Update generation count only for Pro mode
        if (technique === 'Pro') {
          if (user) {
            user.update({
              unsafeMetadata: {
                ...user.unsafeMetadata,
                remove_bg_count: ((user.unsafeMetadata?.remove_bg_count as number) || 0) + 1
              }
            }).catch(err => {
          ...
          <div className="flex-1 flex items-center justify-center p-6 bg-background">
            <Card className="p-8 bg-card border-border max-w-md w-full space-y-6 text-center shadow-2xl ring-1 ring-foreground/5">
              ...
              <p className="text-muted-foreground">
                To keep this tool free and support high-quality 4K generation, please paste your Gemini API key. 
                This supports **every key**, including free ones.
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-background rounded-xl border border-border text-left space-y-2">
                  <div className="flex items-start gap-2 text-xs text-muted-foreground">
                    ...
                    <p>
                      Get your free key from <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-yellow-500 hover:underline">Google AI Studio</a>. 
                      No billing or credit card is required for free tier keys.
                    </p>
                  </div>
                </div>
                
                <Button disabled={!customApiKey.trim()} className="w-full py-6 bg-foreground text-background hover:bg-foreground/90 font-bold rounded-xl">
                  Connect Key
                </Button>
              </div>
            </Card>
              ...
              {!sourceImage ? (
                <div className="flex-1 flex flex-col gap-8">
                  <div className="flex-1 min-h-[300px] border-2 border-dashed border-border rounded-3xl p-12 flex flex-col items-center justify-center text-center space-y-6 relative group hover:border-border/80 transition-all duration-300 bg-card/20">
                    ...
                    <Button className="bg-foreground text-background hover:bg-foreground/90 font-bold rounded-xl px-8 h-12 relative z-20 pointer-events-none">
                      Choose File
                    </Button>
                  </div>
                    ...
                    <div className="relative rounded-[2rem] border border-border/50 bg-card/30 p-2 shadow-2xl backdrop-blur-sm">
                      <div className="relative rounded-3xl overflow-hidden bg-background/50 border border-border/50 flex items-center justify-center"
                        ...
                        <img 
                          src={sourceImage} 
                          alt="Source" 
                          className="max-h-[50vh] max-w-full object-contain rounded-2xl shadow-sm" 
                        />
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center backdrop-blur-[2px]">
                          <Button 
                            variant="destructive" 
                            onClick={() => setSourceImage(null)}
                            className="rounded-xl font-bold shadow-xl scale-95 group-hover:scale-100 transition-transform duration-300"
                          >
                            Remove Image
                          </Button>
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-8 border-t border-border/50">
                <div className="p-6 rounded-3xl bg-card/30 border border-border/50 flex flex-col items-center text-center space-y-4 hover:bg-card/50 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Uses advanced AI to perfectly separate the subject from any background.</p>
                  </div>
                </div>
                <div className="p-6 rounded-3xl bg-card/30 border border-border/50 flex flex-col items-center text-center space-y-4 hover:bg-card/50 transition-colors">
                  <div className="w-12 h-12 rounded-2xl bg-foreground/5 flex items-center justify-center text-foreground">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground mb-1">High Quality</h3>
                    <p className="text-sm text-muted-foreground">Supports up to 4K resolution output with crisp, clean edges.</p>
                  </div>
                </div>
                <div className="p-6 rounded-3xl bg-card/30 border border-border/50 flex flex-col items-center text-center space-y-4 hover:bg-card/50 transition-colors">
                  <div className="w-12 h-12 rounded-2xl bg-foreground/5 flex items-center justify-center text-foreground">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground mb-1">Instant Download</h3>
                    <p className="text-sm text-muted-foreground">Get your transparent PNG instantly, ready to use in your designs.</p>
                  </div>
                </div>
              </div>
            </div>
