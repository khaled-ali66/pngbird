  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (emailCodeFactor) {
        ...
        // Fallback if email_code is not explicitly in supported factors but might be allowed
        try {
          await signIn.prepareFirstFactor({
            strategy: 'email_code',
            emailAddressId: supportedFirstFactors?.[0]?.emailAddressId as string,
          });
          setIsSignUp(false);
          setStep('code');
        } catch (fallbackErr) {
          toast.error('Email code authentication is not supported. Please check your Clerk dashboard settings to enable Email Code.');
        }
      }
    } catch (err: any) {
